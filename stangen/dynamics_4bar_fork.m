...
cog3_B= -BD/2*exp(1i*phi3);                                          cog3_B_x= real(cog3_B);cog3_B_y= imag(cog3_B);
...
cog5_F=  EF/2*exp(1i*phi5);                                          cog5_F_x= real(cog5_F);cog5_F_y= imag(cog5_F);
%3hoek
cog6_G= -1/3*(GH*exp(j*phi6)+FpG*exp(j*phi6)+Fp*exp(j*(phi6-pi/2)));  cog6_G_x= real(cog6_G);cog6_G_y= imag(cog6_G);
cog6_H=  cog6_G + GH*exp(1i*phi6);                                   cog6_H_x= real(cog6_H);cog6_H_y= imag(cog6_H);
...
cog7_J=  cog7_I - IJ*exp(1i*phi7) ;                                  cog7_J_x= real(cog7_J);cog7_J_y= imag(cog7_J);
%3hoek 
...
%   
  ...
  M3 = [zeros(1,2)      cog3_B_y(k)     -cog3_B_x(k)    zeros(1,2)      cog3_D_y(k)     -cog3_D_x(k)    zeros(1,22)];
  ...
  M8 = [zeros(1,16)     -cog8_I_y(k)    cog8_I_x(k)     zeros(1,2)      cog8_K_y(k)     -cog8_K_x(k)    -cog8_L_y(k)    cog8_L_x(k)     zeros(1,4)      -cog8_P_y(k)*cos(phi8(k)-pi/2)+cog8_P_x(k)*sin(phi8(k)-pi/2)    0];
  M9 = [zeros(1,18)     -cog9_J_y(k)    cog9_J_x(k)     zeros(1,4)      -cog9_N_y(k)    cog9_N_x(k)     zeros(1,4)];
  M10 = [zeros(1,22)    cog10_L_y(k)    -cog10_L_x(k)   cog10_N_y(k)    -cog10_N_x(k)   cog10_O_y(k)    -cog10_O_x(k)   zeros(1,2)];
